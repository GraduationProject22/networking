...
def main():
    ...
    while True:
        ...
        for ip in ips:
            ...
            f.close()
            if "W" in obj:
                warning_StringVar.set(obj["W"])
            if count == 0:
                for key in obj:
                    if key == "D":
                        for index, item in enumerate(obj["D"].split(",")):
                            if index != len(obj["D"].split(",")) - 1:
                                vehicle_one_data.append(
                                    list[index] + ":" + item)
...
def gui():
    ...
    warning_StringVar = tk.StringVar()
    ...
    tk.Label(master=frame1, textvariable=warning_StringVar,
             font=("Arial", 22), fg="red").pack(pady=(150, 10))
